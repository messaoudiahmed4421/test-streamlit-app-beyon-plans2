...
a4_data_loader = LlmAgent(
    ...
    description="Charge les résultats A1/A2/A3 et construit le briefing package.",
...
# ════════════════════════════════════════════════════════════════════════════
#  SUB-AGENT 2 : a4_report_writer
#  Utilise UNIQUEMENT google_search (built-in) → pas de conflit.
#  Lit le briefing depuis le state via l'instruction.
# ════════════════════════════════════════════════════════════════════════════
...
a4_report_writer = LlmAgent(
    ...
    model=Gemini(model=MODEL_NAME, retry_options=retry_config),
    description=(
        "Contrôleur de Gestion Senior : rédige le rapport décisionnel stratégique "
        "Markdown à partir du briefing package chargé dans le state."
    ),
...
# ════════════════════════════════════════════════════════════════════════════
#  MAIN AGENT : A4_Strategic_Reporter (SequentialAgent)
#  Orchestre : data_loader → report_writer
# ════════════════════════════════════════════════════════════════════════════

a4_strategic_reporter = SequentialAgent(
    name="A4_Strategic_Reporter",
    description=(
        "Agent stratégique : charge les résultats A1/A2/A3 puis rédige "
        "un rapport décisionnel Markdown pour la direction financière."
    ),
    sub_agents=[a4_data_loader, a4_report_writer],
)

print("✅ A4_Strategic_Reporter défini (SequentialAgent : 2 sous-agents)")
print()
print("  Sub-agent 1 — A4_Data_Loader (LlmAgent)")
print("    └─ Tool : load_analysis_results [custom]")
print("       → Lit state A1/A2/A3, construit briefing package")
print("       → Charge le feedback judge (évaluations passées)")
print("       → Persiste dans state['a4_briefing']")
print()
print("  Sub-agent 2 — A4_Report_Writer (LlmAgent)")
print("    └─ Tool : google_search [built-in ADK]")
print("       → Lit briefing depuis conversation context")
print("       → Intègre le feedback du judge pour corriger les faiblesses")
print("       → Recherche externe si nécessaire (1-3 max)")
print("       → Rédige rapport Markdown → output_key='a4_report'")
print()
print("  Pas de conflit built-in / custom tools ✅")
print("  Boucle de rétroaction A5→A4 activée ✅")

# ════════════════════════════════════════════════════════════════════════════
#  A5_QUALITY_JUDGE — LLM-as-a-Judge
#
#  Évalue la qualité du rapport A4 selon 7 critères objectifs.
#  Détecte les redondances d'anomalies dans le rapport.
#  Charge l'historique des évaluations passées pour s'améliorer.
#  Produit un verdict structuré + feedback d'amélioration pour le workflow.
#
#  Utilise UN SEUL custom tool (load_report_for_judging) → pas de conflit
#  car pas de built-in tool nécessaire.
#
#  Ref. Day 1b §3 — Agent = Model + Instruction + Tools
#  Ref. Day 2a §3 — FunctionTool : structured returns
#  Ref. Day 3a §5 — ToolContext.state : partage inter-agents
#  Ref. Day 3b §7 — Memory Consolidation : mémoire long-terme
# ════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════
#  HELPER : Détection de redondances dans le rapport Markdown
#
#  Analyse le rapport A4 pour identifier les anomalies mentionnées
#  plusieurs fois dans des sections différentes.
# ════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════
#  AGENT A5 : a5_quality_judge (LlmAgent)
# ════════════════════════════════════════════════════════════════════════════

a5_quality_judge = LlmAgent(
    name="A5_Quality_Judge",
    model=Gemini(model=MODEL_NAME, retry_options=retry_config),
    description=(
        "LLM-as-a-Judge : évalue la qualité du rapport A4 selon 7 critères, "
        "détecte les redondances, charge l'historique des évaluations passées, "
        "et produit un verdict structuré avec recommandations d'amélioration."
    ),
...
print("✅ A5_Quality_Judge défini (LlmAgent — LLM-as-a-Judge)")
print()
print("  Tool : load_report_for_judging [custom]")
print("    → Lit state['a4_report'] + state['a4_briefing'] + state['a3_output']")
print("    → Détecte les redondances intra-rapport")
print("    → Charge l'historique des évaluations passées (DB)")
print("    → Retourne le package complet pour cross-check")
print()
print("  Critères d'évaluation (7 dimensions, score 1-10) :")
print("    1. Complétude structurelle")
print("    2. Exactitude des données")
print("    3. Actionnabilité des recommandations")
print("    4. Cohérence analytique")
print("    5. Couverture des anomalies")
print("    6. Qualité rédactionnelle")
print("    7. Non-redondance  ← NOUVEAU")
print()
print("  Mémoire long-terme :")
print("    → Charge les 5 dernières évaluations depuis judge_history (DB)")
print("    → Identifie les problèmes récurrents")
print("    → Vérifie si les faiblesses passées ont été corrigées")
print()
print("  Output : a5_judgment (Markdown) → state['a5_judgment']")

# ── Pipeline séquentiel A1→A2→A3→A4→A5 (Day 1b §4) ─────────────────────
pipeline_a1_a2_a3_a4_a5 = SequentialAgent(
    name="PnL_Analysis_Pipeline",
    description=(
        "Pipeline complet : validation structurelle → "
        "classification comptable → analyse des écarts → "
        "rapport stratégique → évaluation qualité (LLM-as-a-Judge)"
    ),
    sub_agents=[a1_normalizer, a2_classifier, a3_variance_engine, a4_strategic_reporter, a5_quality_judge],
)

print("✅ Pipeline A1→A2→A3→A4→A5 assemblé (SequentialAgent)")
print(f"   Agents : {[ag.name for ag in pipeline_a1_a2_a3_a4_a5.sub_agents]}")
print(f"   A1 (LlmAgent)        → a1_output + a1_summary")
print(f"   A2 (LlmAgent)        → a2_output + a2_summary")
print(f"   A3 (LlmAgent)        → a3_output + a3_summary")
print(f"   A4 (SequentialAgent)  → a4_briefing + a4_report")
print(f"      ├─ A4_Data_Loader   [custom tool]")
print(f"      └─ A4_Report_Writer [google_search]")
print(f"   A5 (LlmAgent)        → a5_judgment (LLM-as-a-Judge)")
print(f"      └─ load_report_for_judging [custom tool]")

# ── Session service (Day 3a §3.2) ─────────────────────────────────────────
if "session_service" not in globals() or session_service is None:
    try:
        session_service = DatabaseSessionService(db_url="sqlite:///pnl_adk_sessions.db")
        print("✅ session_service initialisé (DatabaseSessionService).")
    except Exception as e:
        print(f"⚠️ DatabaseSessionService indisponible ({e}) → fallback InMemorySessionService.")
        session_service = InMemorySessionService()

# ── Runner (Day 3a §3.3) ──────────────────────────────────────────────────
runner_pipeline = Runner(
    agent=pipeline_a1_a2_a3_a4_a5,
    app_name=APP_NAME,
    session_service=session_service,
)

# ── Créer une session (Day 3a §2.4) ──────────────────────────────────────
import uuid as _uuid
_session_id = f"pnl-analysis-run-{_uuid.uuid4().hex[:8]}"
session_pipeline = await session_service.create_session(
    app_name=APP_NAME,
    user_id=USER_ID,
...
user_message = types.Content(
    ...
    parts=[types.Part(text=(
        "Analyser les fichiers P&L : budget_previsionnel.csv, "
        "compte_resultat_reel.csv, chart_of_accounts.csv. "
        "Produire le rapport stratégique complet."
    ))],
)

print("🚀 Lancement du pipeline A1→A2→A3→A4→A5…\n")
print("─" * 70)

events_collected = []
async for event in runner_pipeline.run_async(
    user_id=USER_ID,
    session_id=session_pipeline.id,
...
print("\n" + "─" * 70)
print(f"✅ Pipeline terminé — {len(events_collected)} événement(s) collecté(s).")

# ── Recuperer le session state (Day 3a s5) ────────────────────────────────
session_after = await session_service.get_session(
    app_name=APP_NAME, user_id=USER_ID, session_id=session_pipeline.id
)
state_after = session_after.state
